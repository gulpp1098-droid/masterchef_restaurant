	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (OmnichefModBlocks.RUG_QUEUE.get() == (world.getBlockState(BlockPos.containing(x, y, z))).getBlock()) {
			...
			if (getBlockNBTNumber(world, BlockPos.containing(x, y, z), "queue") > 0) {
				...
				newNBT = NBT - 1;
				receptionString = GetRestaurantStringParameterProcedure.execute(RestaurantIndexSearchByIDProcedure.execute(world, ID), "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name,
				...
				if ((world.getBlockState(BlockPos.containing(receX, receY, receZ))).getBlock() == OmnichefModBlocks.RECEPTION.get() && getBlockNBTNumber(world, BlockPos.containing(receX, receY, receZ), "RestaurantID") == ID) {
					{
				...
				ModifyRestaurantStringParameterProcedure.execute(RestaurantIndexSearchByIDProcedure.execute(world, ID), "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name,
						OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path, "reception", "");
			}
		}
	}
