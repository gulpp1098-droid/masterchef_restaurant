...
public class RugQueueBlockLimitPlaceProcedure {
	public static boolean execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == OmnichefModBlocks.RUG_QUEUE.get()) {
			ReceptionString = GetRestaurantStringParameterProcedure.execute(RestaurantIndexSearchByIDProcedure.execute(world, entity.getData(OmnichefModVariables.PLAYER_VARIABLES).Restaurant_ID), "restaurants",
			...
			RestaurantLevel = GetRestaurantNumberParameterProcedure.execute(RestaurantIndexSearchByIDProcedure.execute(world, entity.getData(OmnichefModVariables.PLAYER_VARIABLES).Restaurant_ID), "restaurants",
