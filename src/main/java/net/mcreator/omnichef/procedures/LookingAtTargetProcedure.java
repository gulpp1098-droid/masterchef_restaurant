...
import net.mcreator.omnichef.network.OverlayPacketMessage;
...
public class LookingAtTargetProcedure {
	...
	private static boolean execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!world.isClientSide()) {
			if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == OmnichefModItems.SPATULA_GOLDEN.get() && (entity.getDisplayName().getString()).equals("Dev")) {
				found = false;
				blockFound = false;
				index = 1;
