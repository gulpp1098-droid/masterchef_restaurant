...
public class PlaceBlockWhenRestaurantOpenProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double z, BlockState blockstate, Entity entity) {
		...
		if (blockstate.getBlock() == OmnichefModBlocks.RUG_QUEUE.get() || blockstate.getBlock() == OmnichefModBlocks.RECEPTION.get() || blockstate.getBlock() == OmnichefModBlocks.CHAIR.get()
				|| blockstate.getBlock() == OmnichefModBlocks.SERVICE_TABLE.get()) {
			if (Level.OVERWORLD == (world instanceof Level _lvl ? _lvl.dimension() : (world instanceof WorldGenLevel _wgl ? _wgl.getLevel().dimension() : Level.OVERWORLD))) {
				if (IsInsideAnyRestaurantProcedure.execute(world, x, z)) {
					if (entity.getData(OmnichefModVariables.PLAYER_VARIABLES).Restaurant_ID > 0) {
						if (!IsInsideRestaurantProcedure.execute(world, x, z, entity.getData(OmnichefModVariables.PLAYER_VARIABLES).Restaurant_ID)) {
				...
				if (blockstate.getBlock() == OmnichefModBlocks.RECEPTION.get() || blockstate.getBlock() == OmnichefModBlocks.RUG_QUEUE.get()) {
					if (entity.getData(OmnichefModVariables.PLAYER_VARIABLES).Restaurant_ID > 0) {
