...
public class CanClaimRestaurantLocationProcedure {
	public static boolean execute(LevelAccessor world, double candidateX, double candidateZ, double restaurantID) {
		...
		for (int _i1 = 0; _i1 < (int) restaurantsArray.size(); _i1++) {
			otherRestaurantID = GetRestaurantNumberParameterProcedure.execute(restaurantIndex, "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name, OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path, "ID");
			if (otherRestaurantID != restaurantID) {
				otherLocationsArray = GetRestaurantArrayParameterProcedure.execute(restaurantIndex, "restaurants", "locations", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name,
