...
public class IsInsideAnyRestaurantProcedure {
	public static boolean execute(LevelAccessor world, double dependencyX, double dependencyZ) {
		...
		for (int _i1 = 0; _i1 < (int) restaurantsArray.size(); _i1++) {
			restaurantID = GetRestaurantNumberParameterProcedure.execute(restaurantIndex, "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name, OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path, "ID");
