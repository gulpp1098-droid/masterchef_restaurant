...
public class ClientQueueWaitStateProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (client.getPersistentData().getDouble("current_position") > 1 && getBlockNBTNumber(world, BlockPos.containing(X, Y, Z), "queue") > 0) {
			...
			TablesArray = GetRestaurantArrayParameterProcedure.execute(RestaurantIndexSearchByIDProcedure.execute(world, restaurantID), "restaurants", "tables", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name,
			...
			if (FoundTable) {
				...
				receptionString = GetRestaurantStringParameterProcedure.execute(RestaurantIndexSearchByIDProcedure.execute(world, restaurantID), "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name,
