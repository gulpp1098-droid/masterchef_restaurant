	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (!world.isClientSide()) {
			...
			if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("SelectingRestaurantLocation")
				...
				if ((world instanceof ServerLevel _level5 ? getEntityFromUUID(_level5, Owner.getData(OmnichefModVariables.PLAYER_VARIABLES).PreviewUUID) : null) != null) {
					if (!(world instanceof ServerLevel _level6 ? getEntityFromUUID(_level6, Owner.getData(OmnichefModVariables.PLAYER_VARIABLES).PreviewUUID) : null).level().isClientSide())
						(world instanceof ServerLevel _level6 ? getEntityFromUUID(_level6, Owner.getData(OmnichefModVariables.PLAYER_VARIABLES).PreviewUUID) : null).discard();
				}
			}
