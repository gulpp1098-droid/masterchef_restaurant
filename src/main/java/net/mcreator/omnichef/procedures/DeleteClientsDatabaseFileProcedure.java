...
public class DeleteClientsDatabaseFileProcedure {
	public static void execute(LevelAccessor world) {
		...
		if (OmnichefModVariables.MapVariables.get(world).LastClientsDatabaseResetDay < currentDay) {
			clientsDatabase = new File(OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path, File.separator + OmnichefModVariables.MapVariables.get(world).ClientsDatabase_File_Name);
			{
				try {
			...
			restaurantFile = new File(OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path, File.separator + OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name);
			{
				try {
			...
			if (DailyResetSaveConfirmedProcedure.execute(world)) {
				OmnichefModVariables.MapVariables.get(world).LastClientsDatabaseResetDay = currentDay;
				OmnichefModVariables.MapVariables.get(world).markSyncDirty();
			} else {
				OmnichefMod.LOGGER.info("DeleteClientsDatabaseFile: daily reset was not saved correctly.");
			}
		}
	}
}