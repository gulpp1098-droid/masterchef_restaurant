			...
			if ((world.getBlockState(BlockPos.containing(x, y, z))).is(BlockTags.create(ResourceLocation.parse("minecraft:beds")))) {
				if (!(world instanceof Level _lvl3 && _lvl3.isDay())) {
					...
					for (int _i1 = 0; _i1 < (int) restaurants.size(); _i1++) {
						if (GetRestaurantLogicParameterProcedure.execute(index, "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name, OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path, "open")
								|| GetRestaurantNumberParameterProcedure.execute(index, "restaurants", OmnichefModVariables.MapVariables.get(world).Restaurant_File_Name, OmnichefModVariables.MapVariables.get(world).Restaurant_Info_Path,
										"active_groups") > 0) {
