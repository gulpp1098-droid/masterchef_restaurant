...
public class GetAmountOfFoodAvailableInTiersProcedure {
	public static double execute(LevelAccessor world, double amountOfTiersDependency, double minTierDependency) {
		...
		FoodFile = new File((FMLPaths.GAMEDIR.get().toString() + "/config/masterchef"), File.separator + MasterchefRestaurantModVariables.MapVariables.get(world).FoodDatabase_File_Name);
		{
			try {
				...
				index = minTierDependency;
				for (int index0 = 0; index0 < (int) amountOfTiersDependency; index0++) {
