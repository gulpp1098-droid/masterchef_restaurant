			...
			if (world.dayTime() % 100 == 0) {
				...
				for (int _i1 = 0; _i1 < (int) RestaurantsOpen.size(); _i1++) {
					...
					if (CloseTime <= world.dayTime() % 24000) {
						...
						GenerateRestaurantMenuProcedure.execute(world, RestaurantsOpen.get((int) index) instanceof Double _doub7 ? _doub7 : 0.0D, GetRestaurantNumberParameterProcedure.execute(RestaurantIndex, "restaurants",
						...
						MasterchefRestaurantModVariables.MapVariables.get(world).RestaurantsOpen
								.remove((int) MasterchefRestaurantModVariables.MapVariables.get(world).RestaurantsOpen.indexOf((RestaurantsOpen.get((int) index) instanceof Double _doub8 ? _doub8 : 0.0D)));
