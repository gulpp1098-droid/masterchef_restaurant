...
public class GetRestaurantNumberParameterProcedure {
	public static double execute(double numberIndex, String arrayNameDependency, String fileNameDependency, String filePathDependency, String propertyDependency) {
		...
		ListOfRestaurants = new File(filePathDependency, File.separator + fileNameDependency);
		{
			try {
				...
				value = Restaurant.get(propertyDependency).getAsDouble();
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
