...
public class DetachQueueRugsProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		SetNumberNBTProcedure.execute(world, X, Y, Z, 0, "RestaurantID");
		CanBreak = true;
		while (CanBreak) {
			CanBreak = false;
			for (Direction directioniterator : Direction.Plane.HORIZONTAL) {
				if (IsQueueRugForRestaurantProcedure.execute(world, X + directioniterator.getStepX(), Y, Z + directioniterator.getStepZ(), ID)
					...
					SetNumberNBTProcedure.execute(world, directioniterator.getStepX() + X, Y, directioniterator.getStepZ() + Z, 0, "RestaurantID");
					...
					SetNumberNBTProcedure.execute(world, directioniterator.getStepX() + X, Y + 1, directioniterator.getStepZ() + Z, 0, "RestaurantID");
					X = directioniterator.getStepX() + X;
					Y = Y + 1;
					Z = directioniterator.getStepZ() + Z;
					NBT = NBT + 1;
