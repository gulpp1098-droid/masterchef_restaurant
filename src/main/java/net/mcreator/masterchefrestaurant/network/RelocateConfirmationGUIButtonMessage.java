...
public record RelocateConfirmationGUIButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	public static final Type<RelocateConfirmationGUIButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(MasterchefRestaurantMod.MODID, "relocate_confirmation_gui_buttons"));
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		guiTools$enhancedImageButton : {
			if (buttonID == 0) {
				net.mcreator.masterchefrestaurant.procedures.ConfirmRestaurantRelocationProcedure.execute(world, entity);
			}
			if (buttonID == 1) {
				net.mcreator.masterchefrestaurant.procedures.ChooseAnotherAreaProcedure.execute(entity);
			}
			if (buttonID == 2) {
				net.mcreator.masterchefrestaurant.procedures.CancelPickAreaProcedure.execute(entity);
			}
		}
	}
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		MasterchefRestaurantMod.addNetworkMessage(RelocateConfirmationGUIButtonMessage.TYPE, RelocateConfirmationGUIButtonMessage.STREAM_CODEC, RelocateConfirmationGUIButtonMessage::handleData);
	}
}